...
def _inf(text, music_config, text_args, model_save_path = None, n_pieces = 1):
    ...
    model = MusicCLIP(music_config, text_args)
    model_save_path = 'model_epoch1000_bs8_lr7.144483820439255e-05_ckpt_epoch3.pth'
    model.load_state_dict(torch.load(model_save_path)["model_state_dict"], strict=False)
    model.eval()
    model.to(device)
